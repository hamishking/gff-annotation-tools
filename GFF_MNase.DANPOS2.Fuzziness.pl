...
use strict;
pod2usage("\nAnnotates GFF file with statistics of nucleosome fuzziness score calculated by DANPOS2 dpos. Requires a list of *.smooth.positions.xls outputs from DANPOS2 with corresponding sample names. NB. Not to be used with differential DANPOS2 files ie ref_adjust or integrative.xls\n
	...
	##Read through each individual GFF file as normal
		...
		print TEMPOUT "$col1\t$col2\t$col3\t$col4\t$col5\t$col6\t$col7\t$col8\t$col9";
				...
				my $label = $namearray[$i];
					chomp $label;
				my $command =  `grep -v "start" $filearray[$i] |  intersectBed -a - -b $tempbedfile | cut -f 6 | st --complete | sed -n 2p `;
				chomp $command;
				my ($number, $min, $q1, $median, $q3, $max, $sum, $mean, $stddev, $stderr)=split (/\t+/,$command); 
			
				##Only keep counts that have 2 or more dyads identified
				if($number >= 1){
				print TEMPOUT ";$label\_DANPOS2Dyads=$number;$label\_DANPOS2.Fuzziness.Median=$median;$label\_DANPOS2.Fuzziness.Mean=$mean;$label\_DANPOS2.Fuzziness.Stdev=$stddev";
				}
				else{
				print TEMPOUT ";$label\_DANPOS2Dyads=0;$label\_DANPOS2.Fuzziness.Median=NA;$label\_DANPOS2.Fuzziness.Mean=NA;$label\_DANPOS2.Fuzziness.Stdev=NA";
				}
